...
Pet project, **v0.0.2 — patch 2**. Под одного пользователя. Без code signing, без telemetry.
...
## Defaults для безопасности

- `max_session_cost_usd` = 1.00 USD — после этого новые AI вызовы блокируются до перезапуска сессии
- `detector_skip_mic` = true — детектор не фаерит на вашем голосе
- `post_meeting_debrief_enabled` = false — opt-in, иначе Sonnet вызовы по умолчанию не идут
- `stealth_enabled` = false — overlay виден в screen-share по умолчанию (включи в Settings если нужно скрыть)
