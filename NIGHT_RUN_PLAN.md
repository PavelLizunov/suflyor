...
**TL;DR (rolling):** 24 releases shipped this session (v0.0.10 → v0.0.34, v0.0.32 folded). Live user feedback drove rapid iteration — F8 crash (real Rust panic from runtime-panics.log, fixed v0.0.22), tile UX (size/transparency/double-click maximize, fixed v0.0.24-25), one-click update (v0.0.23), aggressive-mode opt-in (v0.0.18) with visible 🔥 chip (v0.0.26), percentage-based tile sizing (v0.0.29), Settings sidebar redesign per Claude Design handoff (v0.0.30) + sidebar bottom-pin + overlay width cap (v0.0.31). 255 cargo tests pass through every release · clippy `-D warnings` clean · vite build clean throughout. Three agent-review passes + 1 computer-use live test caught 14 real issues; all fixed inline.

**Releases v0.0.17 → v0.0.34 (this block):**
...
- **v0.0.34** — 3 live-feedback fixes: (1) **Settings footer visually pinned** — `.settings-footer` modifier class with `border-top` + `bg-2` + tighter padding so it READS as fixed instead of floating (was positionally pinned but visually disconnected). (2) **Overlay bar removed 50%-screen cap** — bar can grow to content+50 freely. Was hiding the last chip on 1920p with many active indicators (~1000 px wanted, 960 capped). (3) **Overlay bar manually resizable** — root cause user couldn't drag: ResizeObserver measured `entry.contentRect.width` on `.overlay-root` which stretches to fill window → computed desired ≡ current+50 → setSize echoed → user drag undone next fire. Switched to **grow-only** policy via `overlay-bar.scrollWidth` (intrinsic content). Only setSize when intrinsic > current+4. User can drag wider freely; can't drag below intrinsic (chips would overflow). CSS: added `.overlay-bar > * { flex-shrink: 0 }` so scrollWidth correctly reflects intrinsic (default flex-shrink:1 would defeat the measurement).
