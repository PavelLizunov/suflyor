...
- **Entrypoints (`src/bin/` & `src/`):**
  ...
  - `src/bin/overlay_host/` — **Canonical runtime host directory** (~25 modules). Contains host orchestration logic including `settings_controller.rs`, `tile_controller.rs`, `hotkeys.rs`, `window_lifecycle.rs`, `aux_windows.rs`, `vision_capture.rs`, `recovery.rs`, `wizard.rs`, `diagnostics.rs`, `kbd_shortcuts.rs`, `mlx_lifecycle.rs`, `transcript_player.rs`, and tab-specific settings controllers.
