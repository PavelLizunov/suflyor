...
fn lock_chip_icon_has_an_explicit_vertical_center() {
    ...
    let lock_icon = lock_chip
        .split("source: @image-url(\"../assets/icons/lock.svg\");")
        .nth(1)
        .and_then(|rest| rest.split('}').next())
        .expect("find LockChip lock image");
    ...
    assert!(
        ...
        "LockChip's lock SVG must explicitly stay vertically centered; this catches the top-aligned deep-lock regression in MCP screenshots"
    );
}
