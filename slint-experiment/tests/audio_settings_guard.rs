...
fn audio_settings_refresh_on_reopen_and_keep_macos_separate() {
    ...
    let controller =
        fs::read_to_string(root.join("src/bin/overlay_host/settings_controller.rs")).unwrap();
    let reused = controller
        .split("if let Some(existing) = settings_slot.as_ref()")
        .nth(1)
        .unwrap()
        .split("let win = match SettingsWindow::new()")
        .next()
        .unwrap();
