...
/// Classify a completed switch transaction. Kept pure so the rollback contract
/// is covered without spawning Windows-only llama-server processes in tests.
...
/// `attach_projector = false` so a bad or incompatible projector cannot prevent
/// restoration of the previous text model.
