...
pub fn build_auto_tile_prompts(
    ...
    let answer_rule = if kb_block.is_empty() {
        ...
        "Ответь максимум тремя короткими пунктами. Используй только релевантные факты из проверенной справки; не добавляй фактов или команд от себя. Если вопрос сравнивает термины, явно назови эффект отказа каждого."
    };
    let user_prompt = match trigger {
