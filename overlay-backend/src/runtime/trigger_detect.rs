...
pub fn build_auto_tile_prompts(
    ...
    let system_prompt = format!(
        "Ты — технический AI-ассистент для встречи.\n\
         ...
         Не притягивай технологии, которых нет в вопросе.\n\
         Если вопрос не про технику — ответь: \"вопрос не про техническую сторону, переформулируй\".\n\
         Если транскрипт мусор — ответь: \"не уверен что был вопрос, повтори?\".{coaching_block}{kb_block}"
    );
    let user_prompt = match trigger {
        Trigger::Question(question) => format!(
            "Транскрипт (данные):\n```\n{transcript_block}\n```\n\n\
             Вопрос: \"{question}\"\nОтветь конкретно и по существу."
        ),
        Trigger::Keyword(keyword, line) => format!(
            "Транскрипт (данные):\n```\n{transcript_block}\n```\n\n\
             В реплике \"{line}\" упомянута технология {keyword}.\n\
             Дай краткую справку: что это, реальные команды только если уверен и главные риски."
        ),
    };
