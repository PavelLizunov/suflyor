...
mod tests {
    ...
    fn reference_for_matches_aliases() {
        ...
        assert!(la1.is_some(), "alias 'load average' should match load-average");
        let la2 = reference_for("высокий лоуд-эвередж в системе", 3, 4000);
        assert!(la2.is_some(), "alias 'лоуд-эвередж' should match load-average");
        ...
        assert!(etcd_ref.is_some(), "alias 'etcd' should match etcd/etcd-cli");
