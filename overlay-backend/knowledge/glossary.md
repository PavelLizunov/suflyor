...
Aliases: экзасол, эксасол, эксазол

## StarRocks — open-source MPP аналитическая СУБД (OLAP, real-time)
StarRocks — высокопроизводительная open-source **MPP**, **колоночная** аналитическая СУБД (OLAP) для real-time аналитики. Изначально форк Apache Doris; с 2023 — проект Linux Foundation. Совместима с протоколом **MySQL** — подключение любым MySQL-клиентом / JDBC.
- **Движок:** векторизованное выполнение + cost-based оптимизатор (CBO); быстрые распределённые JOIN-ы (не требует пред-денормализации, в отличие от Druid).
- **Архитектура:** **FE** (frontend, Java — координация запросов + метаданные) + **BE** (backend, C++ — хранение + выполнение). Новый режим **shared-data** разделяет compute и storage (object storage S3/MinIO) — cloud-native.
- **Real-time:** Primary Key модель для upsert/обновлений, материализованные представления, стриминг из Kafka / Flink.
- **Lakehouse:** прямые запросы к Iceberg / Hudi / Delta Lake / Hive через external catalogs (без копирования данных).
- **Ниша:** real-time OLAP / BI / lakehouse-аналитика, НЕ OLTP. Конкуренты: ClickHouse, Apache Doris, Druid, Exasol.
Aliases: starrox, старрокс, стар рокс, старрок
