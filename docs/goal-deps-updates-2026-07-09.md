...
## Фаза 0 — DONE (локальный репозиторий, `045342d`)

`log` уже был 0.4.33; `rusqlite` обновлён до 0.40.1, три мёртвые прямые
зависимости удалены, sidecar помечен `publish = false`, licenses-гейт включён и
прошёл для всех трёх crate. Внешний merge/close Dependabot-PR не выполнялся:
этот charter не даёт полномочий менять GitHub-состояние.
...
## Фаза 1 — DONE (`9b55ad5`): Slint 1.17.1
...
## Фаза 2 — DONE (`af22431`): global-hotkey 0.8.0
...
## Фаза 3 — DONE (`610347a`): rfd 0.17.2
...
## Фаза 4 — DONE (`1448bb4`): reqwest 0.13.4
...
## Фаза 5 — DONE (`c86644b`): rodio 0.22.2 + timestretch 0.5.0
...
## Фаза 6 — DONE (`c64a0bc`): sha2 0.11.0
