...
- **Phase 2b — player (A+B+C): ✅ DONE.** A = pitch-preserving speed via a `timestretch::StreamProcessor` streaming adapter (`StretchSource`, ratio = 1/speed, 1× bit-exact bypass, position clock unchanged); B = speed ComboBox 1×–3× step 0.25 (`speed-index`); C = volume Slider 0–3× (live, no rebuild). Gate 0/0 (3 crates, +StretchSource ratio-inversion test) + independent review SHIP (ratio direction / refill termination / no-panic verified vs crate source; ComboBox `selected` + Slider `changed` confirmed NOT to fire on the programmatic per-session reset). **Pending owner live LISTEN (pitch quality at 2×/3×; M1 = seek-bar may momentarily lead audio at clip start, self-corrects) + VISUAL (control-row layout at min-width, L1)** via `docs/retest-v0.30-player.html`.
