...
- [x] #10 — Shift+F9 не отправляет cloud-запрос без bearer и показывает
  локализованный notice-тайл с подсказкой открыть настройки AI;
- [x] #14 — ожидающий AI semaphore stream завершается при закрытии receiver и
  не начинает уже отменённый HTTP-запрос;
- [x] #16 — оставшиеся UI-строки переведены через `@tr` (PR #38);
- [x] #17 — i18n guard ловит голые литералы в Slint `text`/`placeholder-text`,
  а оставшиеся replay-строки переведены;
...
Полностью закрыты 9 из 22 приоритетных корней, один закрыт частично. Остальные
12 приоритетных пунктов и low/conditional gaps остаются открытыми.
